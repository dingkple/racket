...
;; path-from-start-position: Position Block Path Path Path -> Path 
;; GIVEN: a position, a block and three paths
;; WHERE: 1)the position is the target position to stop
;;        2)the visited is a path containing steps the robot has discovered
;;        3)nps-old is the steps of last result
;;        4)nps is the new steps waiting to be discovered
;; RETURNS: all the steps the robot can discover
;; TERMINATION ARGUMENT: since the robot can only move within a limited range,
;; every recurrence the number of positions have NOT been visited is decreasing
...
(define (path-from-start-position ep bls visited nps-old nps)
  (cond
    [(exist-pos-in-visited? ep nps) visited]
    [else (path-from-start-position-further 
           ep bls
           visited
           nps
           (new-position-add-to-path ep bls visited nps-old nps))]))

;; path-from-start-position-further: Position Block Path Path Path -> Path
;; GIVEN: a position, a block and three paths
;; WHERE: 1)the position is the target position
;;        2)the visited is the steps already found
;;        3)the nps-old is the new steps newly found
;;        4)the nps is the new steps waiting to be discovered
;; RETURNS: all the steps the robot can discover
;; EXAMPLES: (path-from-start-position-further 
;;              (list 2 1) empty 
;;              (list (list (list 1 1) (list 1 1))
;;              (list (list (list 1 1) (list 1 1))
;;              (list (list (list 1 1) (list 1 1))))
...
(define (path-from-start-position-further ep bls visited nps-old nps)
  (if (empty? nps)
      visited
      (path-from-start-position ep bls (set-union visited nps) nps-old nps)))

;; new-position-add-to-path: Position Block Path Path Path -> Path
;; GIVEN: a position, a block and three paths
;; WHERE: 1)the position is the target position
;;        2)the visited is the steps already found
;;        3)the nps-old is the new steps newly found
...
;; EXAMPLES: (new-position-add-to-path 
;;             (list 2 1) empty 
;;             (list (list (list 1 1) (list 1 1)))
;;             (list (list (list 1 1) (list 1 1)))
;;             (list (list (list 1 1) (list 1 1)))) -> 
...
;;                 (list (list 1 1) (list 2 1)))
;; STRATEGY: HOFC
(define (new-position-add-to-path ep bls visited nps-old nps)
  (foldr
   ;; Step Path -> Path
   ;; GIVEN: a step and a path
   ;; RETURNS: the new steps start from the second position of the given
   ;; step
   (lambda (s ans)
     (new-neighbor-add-for-pos s ep bls visited nps-old ans))
   empty
   nps))

;; new-neighbor-add-for-pos: Position Position Block Path Path -> Path
;; GIVEN: a position, another position, a block and two paths
...
;;                   (list (list 1 1) (list 2 1)))
;; STRATEGY: function composition
(define (new-neighbor-add-for-pos p ep bls visited nps-old nps)
  (set-union nps (new-neighbor-for-pos p ep bls nps-old nps)))
...
(define (new-neighbor-for-pos p ep bls nps-old visited)
  (foldr
   ;; Direction Path -> Path
   ...
   (lambda (d ans)
     (if (check-pos 
          (pos-by-move (second p) d) 
          ep bls (set-union nps-old visited))
...
;; return empty iff the start equals the target position
;; EXAMPLES: (path (list 1 1) (list 2 1) empty) -> (list (list "east" 1))
;; STRATEGY: function composition
(define (path sp ep bls)
  (if (equal? sp ep)
      empty
      (path-to-find sp ep bls)))
...
(begin-for-test
  (check-equal? (path (list 1 1) (list 5 2) block-reachable)
                '(("east" 2) ("south" 2) ("west" 1) 
                             ("south" 3) ("east" 3) ("north" 4))
...
(path-to-find (list 1 1) (list 8 6) block-non-reachable)
