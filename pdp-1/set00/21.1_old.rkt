...
(define (get_head_image p)
  (circle (* 23 (get_height_prop (person-height p))) "outline" "red")
  )

(define (get_arm_image p)
  (rectangle (* 70 (get_height_prop (person-height p))) 
...
(define (get_text_posn p)
  (make-posn 190
             (+ 110 
                (/ (image-height (get_head_image p)) 2)
                (* (image-height (get_body_image p)) 1.3)
                ;(* 15 (get_height_prop (person-height p)) 2)
                (image-height (get_leg_image p))
                (* (image-height (get_text_image p)) 2)
                )))
